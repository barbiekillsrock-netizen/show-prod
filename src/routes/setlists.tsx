...
async function shareSetlistPdf(setlist: Setlist, songs: Song[]) {
  ...
  // Concatena PDFs das músicas em ordem
  for (const song of validSongs) {
    try {
      const pdfBytes = song.hasPdf
        ? await (async () => { const b = await getPdfBlob(song.id); return b ? await b.arrayBuffer() : null; })()
        : buildDemoPdfBytes(song);
      if (!pdfBytes) continue;

      const songDoc = await PDFDocument.load(pdfBytes);
      const pages = await merged.copyPages(songDoc, songDoc.getPageIndices());

      // Cabeçalho discreto na primeira página de cada música
      if (pages.length > 0) {
      ...
      // PDF indisponível — pula silenciosamente
    }
  }
