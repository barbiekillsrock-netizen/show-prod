...
async function shareSetlistPdf(setlist: Setlist, songs: Song[]) {
  ...
  const filename = `${setlist.name.replace(/[^a-zA-Z0-9\s]/g, "").trim() || "setlist"}.pdf`;
  const blob = doc.output("blob");
  const file = new File([blob], filename, { type: "application/pdf" });

  // Web Share API — abre menu nativo no tablet/celular
  if (navigator.canShare && navigator.canShare({ files: [file] })) {
    await navigator.share({ files: [file], title: setlist.name });
  } else if (navigator.share) {
    // Fallback: share sem arquivo (alguns browsers)
