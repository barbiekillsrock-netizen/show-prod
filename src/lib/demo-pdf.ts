...
export function buildDemoPdfUrl(song: Song): string {
  ...
  doc.text(
    "Música de demonstração • ShowProd",
    pageW / 2,
...
// Retorna o PDF de demonstração como ArrayBuffer (para merge com pdf-lib)
export function buildDemoPdfBytes(song: Song): ArrayBuffer {
  const doc = new jsPDF({ unit: "pt", format: "a4" });
  const pageW = doc.internal.pageSize.getWidth();
